...
def read_expr_data(data_dir, tissue):
    data_path = os.path.join(data_dir, tissue,"expression.npz")
    print(data_path)
    expr_arr = np.load(data_path)
...
def read_spatial_coord(data_dir, tissue):

    coordinates = pd.read_csv(os.path.join(data_dir,
                                           tissue,
                                           "positions.csv"), delimiter=" ", index_col=None, header=None).values
    return coordinates

def run_model(tissue,spatial=True, feature_dist_thrs=6, spatial_dist_thrs=max_spatial_dist//2):
    args = get_args()
    args.spatial = spatial
    args.dataset = tissue
    thrs_str = "f_%.1f_sp_%.0f" % (feature_dist_thrs, spatial_dist_thrs) if spatial else "%d_non_sp" % sample
    ...
    print(model_fp)
    # coordinates = read_spatial_coord(args.dataset_dir, tissue)
    ...
    expr, genes, samples = read_expr_data(args.dataset_dir, tissue)
...
def plot_pesudo_time(tissue, feature_dist_thrs=6, spatial_dist_thrs=max_spatial_dist//2, fontsz=12, root_idx=4000):
    args = get_args()
    args.dataset = tissue
    lims = [0, 6000]
    ...
    spatial_cords = read_spatial_coord(args.dataset_dir, tissue)
...
if __name__ == "__main__":
    ...
    tissues = ["slideseqv2-mouse-hippocampus", "slideseqv2-mouse-hippocampus", "visium-mouse-brain_sagittal_anterior", "visium-mouse-brain_sagittal_anterior"] #, "Liver""Kidney", "Liver",
    spatials = [True, False, True, False]#False, False,
    for tid, tissue in enumerate(tissues):
       #for root_idx in root_idxs:
        #plot_feature_hist(tissue, samples[tid], args, spatial=False)
        run_model(tissue,spatial=spatials[tid])
